 ...
 *     â€¢ LIVE_POLICY_CHECK = IATokenPolicy.canTransfer() on Monad Testnet
 *     â€¢ FRESH_CLEANVERSE_API_CHECK = Cleanverse REST endpoint (not implemented here)
 *
 ...
 *
 * â”€â”€ Amount used for canTransfer() â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 ...
 * TOKEN_DECIMALS_CANONICAL_HINT (6). Mismatch â†’ DECIMALS_MISMATCH (fail-closed).
 *
 * â”€â”€ Per-holder decision classification â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 ...
 *
 * â”€â”€ Response completeness â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 ...
 * This makes the incomplete reality explicit â€” UNKNOWN is never silently
 * converted to BLOCKED.
 *
 * Response fields:
 *   realityClass      â€” "LIVE_POLICY_CHECK"
 *   policyCallNote    â€” human-readable description of what was called
 *   chainId           â€” 10143 (verified server-side)
 *   isoTime           â€” ISO8601 wall-clock time of response
 *   policy            â€” policy contract address (display only)
 *   token             â€” token address used in canTransfer call (display only)
 *   vault             â€” vault address used as "from" in canTransfer (display only)
 *   tokenDecimals     â€” result of token.decimals() at call time
 *   checkAmount       â€” amount used in eligibility check (raw units string)
 *   hasUnknownResults â€” true if any holder returned UNKNOWN; absent if all resolved
 *   completenessNote  â€” present only when hasUnknownResults is true
 *   holders           â€” array of per-holder results
 *     .holder         â€” holder index (1-5)
 *     .address        â€” holder wallet address
 *     .country        â€” country code from Gate B evidence
 *     .cvRecord       â€” Cleanverse record ID from Gate B evidence
 *     .decision       â€” "ALLOWED" | "BLOCKED" | "UNKNOWN"
 *     .eligible       â€” true | false | null (null for UNKNOWN)
 *     .revertSelector â€” "0x12345678" (only for CALL_EXCEPTION with revert data)
 *     .error          â€” "POLICY_CHECK_UNAVAILABLE" (only for UNKNOWN)
 *
 * Security:
...
// â”€â”€ Error code classification â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
...
function classifyCallError(holderIndex, err) {
  ...
  if (code === EVM_REVERT_CODE) {
    // Genuine EVM policy revert â€” treat as BLOCKED (mirrors _policyAllows semantics).
    ...
    // Sanitized log: only holder index and normalized code â€” no raw message.
  ...
  // Infrastructure/transport/provider failure â€” not a policy decision.
  // Sanitized log: normalized code only â€” no raw err.message or err.shortMessage.
...
export default async function handler(req, res) {
  ...
  try {
      ...
      DEMO_COHORT.map(async (member) => {
        try {
          ...
          if (result === true) {
            ...
            // Returned false (not a revert â€” policy explicitly denies).
            return {
              holder:   member.holder,
              address:  member.address,
              country:  member.country,
    ...
    // Check if any holder returned UNKNOWN â€” make partial reality explicit.
    const hasUnknownResults = results.some(r => r.decision === 'UNKNOWN');

    const responseBody = {
      realityClass: REALITY.LIVE_POLICY_CHECK,
      policyCallNote: 'On-chain IATokenPolicy.canTransfer() via eth_call â€” not a Cleanverse REST API call',
      chainId: CHAIN_ID,
    ...
    if (hasUnknownResults) {
      ...
      responseBody.completenessNote =
        'One or more holders returned UNKNOWN â€” policy check was unavailable for those holders ' +
    ...
    // Sanitized log â€” normalized code only, no raw error message.
